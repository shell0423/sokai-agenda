...
- **公開されるもの**: 実戦リスト（Tier別・株価/PER/PBR・株探リンク）／卒業・決着／注意点／
  **タブ切替・Tier絞り込み・行クリックの銘柄詳細**（JSで実装・Python不要）
- **公開されないもの**: 銘柄検索（トリガー全268社の検索は埋め込みデータが重くなるため）
...
| タブ切替・Tier絞り込み | ✅ | ✅（JS） |
| 行クリック→銘柄詳細 | ✅ | ✅（モーダル） |
| 大量保有チャート | ✅ st.line_chart | ✅ lightweight-charts |
| 議案別賛成率 2025 vs 2026 | ✅ | ✅ |
| 株探・IRBANKリンク | ✅ | ✅ |
| 銘柄検索（全トリガー268社） | ✅ | ❌ |
| 詳細を見られる範囲 | 全銘柄（倉庫を直接引く） | 実戦61社＋除外12社 |

### 公開ページの銘柄詳細（埋め込みデータ）

`src/detail_data.py` が対象73社ぶんの議案・保有推移・トリガー理由を JSON 化し、
`src/dashboard_gen.py` が HTML に埋め込む。チャートは **TradingView lightweight-charts
v4.2.3**（`assets/` に vendor し HTML へインライン。Apache-2.0・ライセンス表記は
ファイル冒頭のまま保持）。外部リソース参照はゼロのまま。

| 内訳 | サイズ |
|---|---|
| ページ本体（表・CSS） | 約 40KB |
| 詳細データ JSON（73社） | 約 100KB |
| lightweight-charts | 160KB |
| **合計** | **約 470KB** |

> **保有推移の時点は月まで**。元データ `trigger_holdings.csv` が `24/03` 形式のため、
> チャート内部では同一月の点に連番の日を振って並び順だけを保ち、
> **軸・ツールチップには `24/03` と年月のみ**を表示する（存在しない日を見せない）。
